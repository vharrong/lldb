//===-- NativeRegisterContextAndroid_x86_64.cpp ---------------*- C++ -*-===//
//
...
#include "NativeRegisterContextAndroid_x86_64.h"
...
#include "Plugins/Process/Android/NativeProcessAndroid.h"
...
// ----------------------------------------------------------------------------
// NativeRegisterContextAndroid_x86_64 members.
// ----------------------------------------------------------------------------

NativeRegisterContextAndroid_x86_64::NativeRegisterContextAndroid_x86_64 (NativeThreadProtocol &native_thread, uint32_t concrete_frame_idx, RegisterInfoInterface *reg_info_interface_p) :
...
uint32_t
NativeRegisterContextAndroid_x86_64::GetRegisterSetCount () const
{
    uint32_t sets = 0;
...
NativeRegisterContextAndroid_x86_64::GetRegisterSet (uint32_t set_index) const
{
    if (!IsRegisterSetAvailable (set_index))
        return nullptr;
...
lldb_private::Error
NativeRegisterContextAndroid_x86_64::ReadRegisterRaw (uint32_t reg_index, RegisterValue &reg_value)
{
    Error error;
    ...
    NativeProcessAndroid *const process_p = reinterpret_cast<NativeProcessAndroid*> (process_sp.get ());
    if (!process_p->ReadRegisterValue(m_thread.GetID(),
                                     reg_info->byte_offset,
                                     reg_info->name,
                                     reg_info->byte_size,
                                     reg_value))
        error.SetErrorString ("NativeProcessAndroid::ReadRegisterValue() failed");

    return error;
}

lldb_private::Error
NativeRegisterContextAndroid_x86_64::ReadRegister (const RegisterInfo *reg_info, RegisterValue &reg_value)
{
    Error error;
...
lldb_private::Error
NativeRegisterContextAndroid_x86_64::WriteRegister(const uint32_t reg,
                                                 const RegisterValue &value)
{
    ...
    {
        error.SetErrorStringWithFormat ("NativeRegisterContextAndroid_x86_64::%s failed to get RegisterInfo for write register index %" PRIu32, __FUNCTION__, reg_to_write);
        return error;
    }
    ...
    if (!process_p->WriteRegisterValue(m_thread.GetID(),
                                       ...
                                       value_to_write))
        error.SetErrorString ("NativeProcessAndroid::WriteRegisterValue() failed");

    return error;
}

lldb_private::Error
NativeRegisterContextAndroid_x86_64::WriteRegister (const RegisterInfo *reg_info, const RegisterValue &reg_value)
{
...
lldb_private::Error
NativeRegisterContextAndroid_x86_64::ReadAllRegisterValues (lldb::DataBufferSP &data_sp)
{
    Error error;
    ...
    else if (GetFPRType () == eFPRTypeXSAVE)
    {
        ...
        for (uint32_t reg = m_reg_info.first_ymm; reg <= m_reg_info.last_ymm; ++reg)
        {
            ...
            {
                error.SetErrorStringWithFormat ("NativeRegisterContextAndroid_x86_64::%s CopyXSTATEtoYMM() failed for reg num %" PRIu32, __FUNCTION__, reg);
                return error;
            }
        }
...
lldb_private::Error
NativeRegisterContextAndroid_x86_64::WriteAllRegisterValues (const lldb::DataBufferSP &data_sp)
{
    Error error;

    if (!data_sp)
    {
        error.SetErrorStringWithFormat ("NativeRegisterContextAndroid_x86_64::%s invalid data_sp provided", __FUNCTION__);
        return error;
    }

    if (data_sp->GetByteSize () != REG_CONTEXT_SIZE)
    {
        error.SetErrorStringWithFormat ("NativeRegisterContextAndroid_x86_64::%s data_sp contained mismatched data size, expected %" PRIu64 ", actual %" PRIu64, __FUNCTION__, REG_CONTEXT_SIZE, data_sp->GetByteSize ());
        return error;
    }
    ...
    if (src == nullptr)
    {
        error.SetErrorStringWithFormat ("NativeRegisterContextAndroid_x86_64::%s DataBuffer::GetBytes() returned a null pointer", __FUNCTION__);
        return error;
    }
    ...
    if (!WriteGPR ())
    {
        error.SetErrorStringWithFormat ("NativeRegisterContextAndroid_x86_64::%s WriteGPR() failed", __FUNCTION__);
        return error;
    }
    ...
    if (!WriteFPR ())
    {
        error.SetErrorStringWithFormat ("NativeRegisterContextAndroid_x86_64::%s WriteFPR() failed", __FUNCTION__);
        return error;
    }
    ...
    if (GetFPRType() == eFPRTypeXSAVE)
    {
        ...
        for (uint32_t reg = m_reg_info.first_ymm; reg <= m_reg_info.last_ymm; ++reg)
        {
            ...
            {
                error.SetErrorStringWithFormat ("NativeRegisterContextAndroid_x86_64::%s CopyYMMtoXSTATE() failed for reg num %" PRIu32, __FUNCTION__, reg);
                return error;
            }
        }
    }
...
bool
NativeRegisterContextAndroid_x86_64::IsRegisterSetAvailable (uint32_t set_index) const
{
...
lldb::ByteOrder
NativeRegisterContextAndroid_x86_64::GetByteOrder() const
{
...
bool
NativeRegisterContextAndroid_x86_64::IsGPR(uint32_t reg_index) const
{
    // GPRs come first.
    return reg_index <= m_reg_info.last_gpr;
}

NativeRegisterContextAndroid_x86_64::FPRType
NativeRegisterContextAndroid_x86_64::GetFPRType () const
{
    if (m_fpr_type == eFPRTypeNotValid)
    {
        ...
        if (!const_cast<NativeRegisterContextAndroid_x86_64*> (this)->ReadFPR ())
        {
...
bool
NativeRegisterContextAndroid_x86_64::IsFPR(uint32_t reg_index) const
{
    return (m_reg_info.first_fpr <= reg_index && reg_index <= m_reg_info.last_fpr);
}

bool
NativeRegisterContextAndroid_x86_64::IsFPR(uint32_t reg_index, FPRType fpr_type) const
{
...
bool
NativeRegisterContextAndroid_x86_64::WriteFPR()
{
    NativeProcessProtocolSP process_sp (m_thread.GetProcess ());
    if (!process_sp)
        return false;
...
bool
NativeRegisterContextAndroid_x86_64::IsAVX(uint32_t reg_index) const
{
    return (m_reg_info.first_ymm <= reg_index && reg_index <= m_reg_info.last_ymm);
}

bool
NativeRegisterContextAndroid_x86_64::CopyXSTATEtoYMM (uint32_t reg_index, lldb::ByteOrder byte_order)
{
    if (!IsAVX (reg_index))
        return false;
...
bool
NativeRegisterContextAndroid_x86_64::CopyYMMtoXSTATE(uint32_t reg, lldb::ByteOrder byte_order)
{
    if (!IsAVX(reg))
        return false;
...
bool
NativeRegisterContextAndroid_x86_64::ReadFPR ()
{
    NativeProcessProtocolSP process_sp (m_thread.GetProcess ());
    if (!process_sp)
        return false;
...
bool
NativeRegisterContextAndroid_x86_64::ReadGPR()
{
    NativeProcessProtocolSP process_sp (m_thread.GetProcess ());
    if (!process_sp)
        return false;
...
bool
NativeRegisterContextAndroid_x86_64::WriteGPR()
{
    NativeProcessProtocolSP process_sp (m_thread.GetProcess ());
    if (!process_sp)
        return false;
